        ...
        <div className="grid lg:grid-cols-2 gap-12 lg:gap-16 items-center min-h-[70vh]">
          <motion.div style={{ y: textY, opacity }}>
            ...
            <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7, delay: 1.1 }}
              ...
              <span className="text-cyan-400/80 text-xs uppercase tracking-[0.15em] font-semibold" style={{ fontFamily: 'var(--font-sans)' }}>Luxury Pool Specialists</span>
            </motion.div>
            ...
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7, delay: 1.3 }} className="flex flex-wrap gap-4 mt-10">
              <Link to="/contact" className="group inline-flex items-center gap-3 bg-cyan-500 text-slate-950 px-8 py-4 text-sm uppercase tracking-[0.15em] font-semibold hover:bg-cyan-400 hover:shadow-xl hover:shadow-cyan-500/20 transition-all duration-500" style={{ fontFamily: 'var(--font-sans)' }}>
          ...
          <motion.div initial={{ opacity: 0, x: 60 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 1.2, delay: 0.4 }} className="relative hidden lg:block">
            <div className="relative">
              ...
              <div className="absolute bottom-6 left-6 bg-slate-950/90 border border-cyan-500/20 px-5 py-3 backdrop-blur-sm">
                <div className="flex items-center gap-3">
                  ...
                  <span className="text-white/60 text-xs uppercase tracking-[0.2em]" style={{ fontFamily: 'var(--font-sans)' }}>Crystal Clear Quality</span>
                </div>
              </div>
            </div>
          </motion.div>
...
function TrustStrip() {
  ...
  const inView = useInView(ref, { once: true, margin: '-50px' });
  return (
    <section ref={ref} className="bg-slate-900 border-y border-cyan-500/10 py-10 sm:py-14">
        ...
        <div className="grid md:grid-cols-3 gap-4 sm:gap-6">
          {tiers.map((tier, i) => (
            <motion.div key={tier.name} initial={{ opacity: 0, y: 30 }} animate={inView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.7, delay: i * 0.12 }}
              className={`relative p-8 border ${tier.highlight ? 'border-cyan-500/40 bg-cyan-500/5' : 'border-white/5 bg-slate-900/50'}`}>
              {tier.highlight && <div className="absolute top-0 left-0 right-0 h-[2px] bg-cyan-500" />}
              {tier.highlight && <div className="absolute -top-3 right-6"><span className="bg-cyan-500 text-slate-950 text-[10px] uppercase tracking-[0.2em] px-3 py-1 font-bold" style={{ fontFamily: 'var(--font-sans)' }}>Popular</span></div>}
            ...
            return (
              <motion.div key={service.title} initial={{ opacity: 0, y: 30 }} animate={inView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.7, delay: 0.08*i }} className={i === 0 ? 'sm:col-span-2 lg:col-span-2' : ''}>
                <Link to={`/services#${services?.items?.[i]?.slug || ''}`} className={`group relative block overflow-hidden ${i === 0 ? 'aspect-[16/9] sm:aspect-[2/1]' : 'aspect-[3/4]'}`}>
                  ...
                  <div className="absolute top-5 left-5 z-10 w-10 h-10 bg-cyan-500 flex items-center justify-center group-hover:bg-cyan-400 transition-colors"><Icon size={18} weight="fill" className="text-slate-950" /></div>
          ...
          {projects.items.map(project => (
            <div key={project.slug} className="group relative flex-shrink-0 w-[300px] sm:w-[360px] lg:w-[420px] overflow-hidden">
              <div className="aspect-[3/4] overflow-hidden relative">
                ...
                <div className="absolute top-5 left-5 z-10"><span className="bg-cyan-500 text-slate-950 text-[10px] uppercase tracking-[0.2em] px-3 py-1.5 font-semibold" style={{ fontFamily: 'var(--font-sans)' }}>{project.category}</span></div>
              </div>
        ...
        <div className="grid lg:grid-cols-2 gap-16 lg:gap-24 items-center">
          <motion.div initial={{ opacity: 0, x: -40 }} animate={inView ? { opacity: 1, x: 0 } : {}} transition={{ duration: 0.9 }}>
            ...
            <div className="flex gap-10 sm:gap-16">
              {[{ v: business.established, l: 'Founded' }, { v: business.projectsCompleted, l: 'Created' }, { v: '24/7', l: 'Operations' }].map(s => (
          ...
          <motion.div initial={{ opacity: 0, x: 40 }} animate={inView ? { opacity: 1, x: 0 } : {}} transition={{ duration: 0.9, delay: 0.2 }} className="relative">
            ...
            <div className="absolute -top-4 -right-4 sm:-right-6 bg-cyan-500 text-slate-950 p-5 sm:p-7 shadow-2xl"><div className="text-center"><div className="text-xs uppercase tracking-[0.2em]" style={{ fontFamily: 'var(--font-sans)' }}>Est.</div><div className="font-heading text-3xl sm:text-4xl leading-none mt-1">{business.established}</div></div></div>
    ...
    <section ref={ref} className="relative py-24 sm:py-32 lg:py-40 overflow-hidden">
      ...
      <div className="relative z-10 max-w-4xl mx-auto px-5 sm:px-8 lg:px-12 text-center">
        <motion.div initial={{ opacity: 0, y: 30 }} animate={inView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.9 }}>
          <div className="w-12 h-[2px] bg-cyan-500 mx-auto mb-6" />
          <h2 className="font-heading text-white leading-[0.95] mb-6" style={{ fontSize: 'clamp(2rem, 5vw, 3.5rem)' }}>Ready for <span className="text-cyan-500">Luxury Living?</span></h2>
          ...
          <div className="flex flex-col sm:flex-row gap-4 justify-center">
            <Link to="/contact" className="group inline-flex items-center gap-3 bg-cyan-500 text-slate-950 px-8 py-4 text-sm uppercase tracking-[0.15em] font-semibold hover:bg-cyan-400 transition-all duration-500" style={{ fontFamily: 'var(--font-sans)' }}>
              Free Design Consult <ArrowRight size={16} className="group-hover:translate-x-1 transition-transform" />
            </Link>
